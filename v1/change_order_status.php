<?php
  ...
  if($_SERVER['REQUEST_METHOD'] === "POST"){
    ...
    if(!isset($_GET['jwt']) || empty($data->id) || empty($data->status)) {
    ...
    try{
        ...
        $data_arr = $od->changeOrderStatus($data->id,$data->status);
        http_response_code(200);
        echo json_encode(array("status" => 200, "message" =>"Updated Order Status"));
        return;
